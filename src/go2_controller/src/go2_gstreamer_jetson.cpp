...
Go2GstreamerJetsonNode::Go2GstreamerJetsonNode()
: Node("go2_gstreamer_jetson"),
  last_publish_ns_(0)
{
    DeclareParameters();
    ...
    pub_ = this->create_publisher<sensor_msgs::msg::CompressedImage>(image_topic, 3);
    ...
    RCLCPP_INFO(this->get_logger(), "GStreamer Jetson pipeline: %s", pipeline.c_str());
    ...
    running_ = true;
    reader_thread_ = std::thread(&Go2GstreamerJetsonNode::ReaderLoop, this);

    timer_ = this->create_wall_timer(
        std::chrono::duration<double>(timer_period),
        std::bind(&Go2GstreamerJetsonNode::TimerCallback, this));

    RCLCPP_INFO(this->get_logger(), "GStreamer Jetson node started, publishing to %s", image_topic.c_str());
}
...
void Go2GstreamerJetsonNode::DeclareParameters()
{
...
std::string Go2GstreamerJetsonNode::BuildPipeline()
{
    std::string m_addr = this->get_parameter("multicast_address").as_string();
    ...
    return "udpsrc address=" + m_addr + " port=" + std::to_string(port) +
        " multicast-iface=" + iface + " buffer-size=" + std::to_string(buf_size) +
...
void Go2GstreamerJetsonNode::ReaderLoop()
{
    while (running_) {
        cv::Mat frame;
...
void Go2GstreamerJetsonNode::TimerCallback()
{
    cv::Mat frame;
    {
        std::lock_guard<std::mutex> lock(frame_mutex_);
    ...
    if (output_width_ > 0 && output_height_ > 0) {
        cv::resize(frame, frame, cv::Size(output_width_, output_height_), 0, 0, cv::INTER_LINEAR);
    }
